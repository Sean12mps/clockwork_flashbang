...
function childthemes_register_scripts(){   
    ...
    wp_register_script('childthemes-cookie', CHILD_JS_URL . '/jquery.cookie.js', array('jquery'));
...
function childthemes_load_script(){
    wp_enqueue_style('childthemes-animate', CHILD_CSS_URL . '/animate.css');
    
	$clockwork_support = array( 	
							'name'		=>['header_teaser'], 
							'selector'	=>['#header-title'], 
							'functions'	=>['header_teaser'],
							'locations' =>['all'],
							'site_url' 	=>get_site_url()
						);
	wp_localize_script( 'childthemes-functions', 'clockwork_support', $clockwork_support );
	wp_enqueue_script( 'childthemes-clockwork' );
	wp_enqueue_script( 'childthemes-cookie' );
